...
/**
 Standard zoom proportions for an axis.

 - `SChartAxisZoomLevelOriginal` The default range.
 - `SChartAxisZoomLevelDouble` Double the current zoom level.
 - `SChartAxisZoomLevelHalf` Half the current zoom level.
 */
...
/**
 Defines how discontinuous axes deal with tick label clipping over discontinuities

 When there are labels either side of a discontinuity which overlap, defines whether the higher or lower label will be removed.
...
/**
 ...
 More information about using `SChartAxis` can be found in the [user guide](axes-and-tickmarks.html).



 @see `SChartNumberAxis` <br/> `SChartDateTimeAxis` <br/> `SChartDiscontinuousNumberAxis` <br/> `SChartDiscontinuousDateTimeAxis` <br/> `SChartCategoryAxis` <br/> `SChartLogarithmicAxis` <br/> `SChartAxisStyle`
 */
