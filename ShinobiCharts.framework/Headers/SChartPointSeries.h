...
/** An `SChartPointSeries` acts as the base class for both `SChartScatterSeries` and `SChartBubbleSeries` and concerns itself with drawing data points. `SChartScatterSeries` adds the drawing of inner points, whereas `SChartBubbleSeries` adds the ability to control the drawn datapoint size via `SChartBubbleDataPoint` objects. It is therefore unlikely that you will ever want to directly instantiate a series of this type, but should you wish to do so note that the corresponding point style is `SChartBasePointStyle`. */
