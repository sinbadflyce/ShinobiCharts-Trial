...
/** Defines what should be selected from an `SChartSeries` when the user taps the chart.
 
 The selection modes are defined as follows:
 
 - `SChartSelectionNone` Select nothing when tapped.
 - `SChartSelectionPoint` Select a point when it is tapped.
 - `SChartSelectionSeries` Select the series when it is tapped.
 */
...
/** The orientation of the `SChartAxis` which is associated with an `SChartSeries`.
 
 - `SChartSeriesOrientationHorizontal`: The orientation of the chart axis which is associated with the series is horizontal. Thus the controlled variable will be drawn on the x-axis, while the measured variable will be drawn on the y-axis.
 - `SChartSeriesOrientationVertical`: The orientation of the chart axis which is associated with the series is vertical. Thus the  controlled variable will be drawn on the y-axis, while the measured variable will be drawn on the x-axis.
...
/** 
 ...
 More information about using `SChartSeries` can be found in the [user guide](series.html).
...
/** This method returns a string value that will represent the value of a given data point based on its key.
 ...
 @param axis The axis that the value corresponds to, where `nil` represents an axis independent value.
